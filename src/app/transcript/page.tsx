...
import { Transcript2 } from './record.d';
...
export default function Transcript() {
    ...
    const columns: string[] = ["Nama", "NIM", "Kode MK 1", "Nama MK 1", "Nilai 1", "SKS 1", "Kode MK 2", "Nama MK 2", "Nilai 2", "SKS 2", "Kode MK 3", "Nama MK 3", "Nilai 3", "SKS 3", "Kode MK 4", "Nama MK 4", "Nilai 4", "SKS 4", "Kode MK 5", "Nama MK 5", "Nilai 5", "SKS 5", "Kode MK 6", "Nama MK 6", "Nilai 6", "SKS 6", "Kode MK 7", "Nama MK 7", "Nilai 7", "SKS 7", "Kode MK 8", "Nama MK 8", "Nilai 8", "SKS 8", "Kode MK 9", "Nama MK 9", "Nilai 9", "SKS 9", "Kode MK 10", "Nama MK 10", "Nilai 10", "SKS 10", "IPK", "Public Key", "Modulus Key", "Signature", "Encrypt Key", "Unduh", "Verifikasi"];
    const filledColumns: string[] = ["nama", "nim", "kodeMk1", "namaMk1", "nilai1", "sks1", "kodeMk2", "namaMk2", "nilai2", "sks2", "kodeMk3", "namaMk3", "nilai3", "sks3", "kodeMk4", "namaMk4", "nilai4", "sks4", "kodeMk5", "namaMk5", "nilai5", "sks5", "kodeMk6", "namaMk6", "nilai6", "sks6", "kodeMk7", "namaMk7", "nilai7", "sks7", "kodeMk8", "namaMk8", "nilai8", "sks8", "kodeMk9", "namaMk9", "nilai9", "sks9", "kodeMk10", "namaMk10", "nilai10", "sks10", "ipk", "publicKeyE", "publicKeyN", "signature", "encryptKey"];

    const handleValidate = (key: number) => {
        ...
        const signatureDigest = RSA.decryptText(transcript[key].signature, transcript[key].publicKeyE, transcript[key].publicKeyN);

        // Langkah 2. Hash database
        const messageDigest = keccak256(String(transcript[key]));

        // Langkah 3. Compare message digest
        if (signatureDigest === messageDigest) {
            transcript[key].validated = 'valid';
        } else {
            transcript[key].validated = 'invalid';
        }
    }
                            ...
                            <tbody>
                                {transcript.map((item: any, idx) => {
                                    return <tr key={`row-${idx}`} className="text-purple1 hover:bg-[#9CB3DA]">
                                        ...
                                        <GeneratePdfForm transcriptData={item.transcriptData} />
                                        </th>
                                        <th className="border-2 border-purple1 px-6 py-1 w-24 font-normal ">
                                            <div className="flex justify-center gap-x-2">
                                            <img src={item.validated === "unchecked" ? "/unchecked.png" : item.validated === "valid" ? "/valid.png" : "/invalid.png"} alt="download file" className="w-8 h-6 "></img>
