...
export default function Transcript() {
    ...
    const decryptTranscript = (transcript: RecordType[]): RecordType[] => {
        // console.log("Miaw Auug", transcript);
        return transcript.map(item => {
            const decryptedItem = { ...item };
            // console.log("wkwk", decryptedItem);
            Object.keys(decryptedItem).forEach(key => {
                if (key !== 'signature' && key !== 'encryptKey' && key !== 'keyId') {
                    decryptedItem[key] = RC4.decrypt(Base64.decode(decryptedItem[key]), decryptedItem["encryptKey"]);
                }
                // console.log("nginx", decryptedItem[key]);
            });
            // console.log("tes", decryptedItem);
                                ...
                                {transcript.map((item: any, idx) => {
                                    return <tr key={`row-${idx}`} className="text-purple1 hover:bg-[#9CB3DA]">
                                        ...
                                        <th className="border-2 border-purple1 px-6 py-1 w-24 font-normal ">
                                            ...
                                            <button key="idx" className="bg-pink1 border-2 border-white1 text-white1 text-sm rounded-lg block px-2 drop-shadow-lg hover:drop-shadow-md hover:bg-pink2 pr-1"
